...
void dateSkip2()
{
    ...
    Result ts = timeSetCurrentTime(TimeType_NetworkSystemClock, (uint64_t)(insert_date_here)); //Set new time, by replacing it with `sed`..
    if (R_FAILED(ts))
        fatalThrow(ts);
}
...
{
    Result ts = timeSetCurrentTime(TimeType_NetworkSystemClock, date); //Set new time, by replacing it with `sed`..
    if (R_FAILED(ts))
        fatalThrow(ts);
}
